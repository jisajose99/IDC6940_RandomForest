...
accuracy <- sum(prediction==test$HeartDisease) / nrow(test)
accuracy
```
...
conf_plot <- conf_mat(results_df, truth = truth, estimate = prediction)
conf_data <- as.data.frame(conf_plot$table)
names(conf_data) <- c("Prediction", "Truth", "Freq")

ggplot(conf_data, aes(x = Truth, y = Prediction, fill = Freq)) +
