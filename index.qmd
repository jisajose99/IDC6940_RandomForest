...
# Print the best model and plot tuning results
#print(rf_tuned)
#plot(rf_tuned)
...
cm <- confusionMatrix(predictions, test$HeartDisease)
...
- Accuracy: 88.34% <br/> 
- Sensitivity (Recall for ‘No’ class): 95.73% <br/> 
- Specificity: 80.19%  <br/> 
- Kappa: 0.76 (substantial agreement) <br/> 
- AUC: 0.9371

These numbers demonstrate that the tuned model not only does well on the training folds but can also be expected to perform well on unseen data. As shown in Figure 1, the ROC curve indicates strong separation between classes, emphasizing a steep rise towards the top left corner which implies high sensitivity and low false positive rate (Saito & Rehmsmeier, 2015).

Figure 6 below The tuned random forest model’s ROC curve with an AUC score of 0.9371. This score suggests remarkable classification ability. 

```{r}

# ROC Curve and AUC
roc_obj <- roc(response = test$HeartDisease, predictor = rf_probs, levels = c("No", "Yes"))
...
varImpPlot(rf_model,
           type = 2,  # Type 2 = MeanDecreaseGini
...
In an ensemble model such as the random forest, a lack of tuning may still produce acceptable results, however, it will never be fully optimal. For instance, mtry was tuned such that the trees in the forest had the optimal levels of randomness along with the optimal depth of the splits so that the model could generalize to new data without overfitting (Boulestix et al., 2012). 
